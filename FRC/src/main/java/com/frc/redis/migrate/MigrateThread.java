...
public class MigrateThread implements Runnable {
    ...
    private final boolean updateZKRedis(RedisClient client) {
        ...
        zkClient.updatePath(logIndex, path, JSONProvider.toJSONString(logIndex, redisInfo));
        FRCLogger.getInstance().info(logIndex, flag, "path:{}  data:{}", null, path,
        ...
        try {
            ...
            switch (redisClient.getRedis_state()) {
                case RedisState.STATE_ADD:
                    ...
                    RedisInfo redisInfo = ClusterAlgo.redisClient2Info(logIndex, redisClient);
    ...
    private final ScanResult<String> listKeys(RedisClient client, int cursor) {
        String flag = this.getClassName() + ".listKeys";
        try {

            return client.scan(logIndex, cursor);
        } catch (Exception e) {
    ...
    private void work() {
        ...
        try {
            ...
            if (null == iteratorMap || iteratorMap.isEmpty())
                return;
                ...
                switch (client.getRedis_state()) {
                    ...
                    case RedisState.STATE_ADD:
                        ...
                        if (!this.getLock(client)) {
                            FRCLogger.getInstance().info(logIndex, flag,
                                "{} has been another locked", null, client.getRedis_name());
                            continue;
                        }

...
public class ClusterAlgo {
	...
	lock.lock();
	try{
	    if(curr.getRedis_state() == RedisState.STATE_NORMAL) return curr;
	    ...
	    FRCLogger.getInstance().warn(logIndex, flag, "key:{} source:{}  dest:{}", e, key, JSONProvider.toJSONString(logIndex, curr));
	    return null; 
	}finally{
	    lock.unlock();
	}
	...
	if (null == redisClient
		|| StringUtils.isBlank(redisClient.getRedis_name())) {
	    return;
	}
	String flag = getClassName() + ".removeRedis";
	try {
	    ...
	    List<Long> listVnodes = mapRedisVnodeList.get(redis_name);
	    if (null == listVnodes || listVnodes.isEmpty()){
		FRCLogger.getInstance().info(logIndex, flag, "not found redis:{}", null, redis_name);
		return;
		}

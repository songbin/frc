...
public class PropertiesLoader {
    ...
    public static synchronized PropertiesLoader getIntance() {
        if (null == instance) {
            ...
            boolean ret = properties.loadConfig(DefaultValues.PATH_PROPERTIES_DEFAULT);
            if (!ret) {
                FRCLogger.getInstance().warn(-100, "PropertiesLoader.getIntance",
    ...
    public boolean getRedisTestBorrow() {
        return 1 == properties.getInt(ConfigKeys.REDIS_TEST_BORROW, 1) ? true : false;
    }

    public boolean getRedisTestReturn() {
        return 1 == properties.getInt(ConfigKeys.REDIS_TEST_RETURN, 1) ? true : false;
    }

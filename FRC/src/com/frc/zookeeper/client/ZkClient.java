...
public class ZkClient {
    ...
    private static String ZK_ROOT = PropertiesLoader.getIntance()
	    .getZookerRoot();
	...
	try {
	    ...
	    FRCLogger.getInstance().warn(logIndex, flag, "path:{}  exception",
		    e, path);
	    return null;
	}
	...
	if (null == client) {
	    FRCLogger.getInstance().warn(logIndex, flag,
		    "create zookeeper client fail!", null);
	    return false;
	}
	...
	if (!this.startWatch(logIndex, client)) {
	    FRCLogger.getInstance().warn(logIndex, flag, "start watch fail!",
		    null);
	    client.close();
	    return false;
	}
	...
	try {
	    ...
	    FRCLogger.getInstance().warn(logIndex, flag,
		    "ZkWatchRunnable thread run fail!", e);
	    return false;
	}
	return true;
    }
     ...
     * node，2：success
     */
    private int nodeAdd(long logIndex, CuratorFramework client,
	...
	try {
	    ...
	    RedisInfo redisInfo = (RedisInfo) JSONProvider.parseObject(
		    logIndex, strVal, RedisInfo.class);
	    if (null == redisInfo) {
		FRCLogger.getInstance().warn(logIndex, flag, ":{}", null,
			strVal);
	    ...
	    FRCLogger
		    ...
		    .warn(logIndex,
			    flag,
			    "nodeAdd event :path:{}->[{}] add fail,exception detail: {}",
			    e, strVal);
	    return DefaultValues.ZK_ADD_FAIL;
	}
	...
	try {
		...
		FRCLogger.getInstance().warn(logIndex, flag,
			"redis workstate is wrong! data:{}", null,
	...
	try {
	    String redisStr = new String(ZkClient.client.getData().forPath(
		    event.getData().getPath()));
	    // jsonRedis = Utils.str2Json(logIndex, redisStr);
	    RedisInfo redisCli = (RedisInfo) JSONProvider.parseObject(logIndex,
		    redisStr, RedisInfo.class);
	    return redisCli;
	} catch (Exception e) {
